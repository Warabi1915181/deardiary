...
struct WashiTape: View {
  ...
  var body: some View {
    Rectangle()
      .fill(Color("RomanceBackground").opacity(0.85))
      .frame(width: 72, height: 22)
      .overlay(
        Rectangle()
          .stroke(Color("RomanceForeground").opacity(0.12), lineWidth: 1)
      )
      .rotationEffect(.degrees(angle))
      .shadow(color: Color(red: 0.45, green: 0.3, blue: 0.25).opacity(0.15), radius: 2, x: 0, y: 1)
  }
}
